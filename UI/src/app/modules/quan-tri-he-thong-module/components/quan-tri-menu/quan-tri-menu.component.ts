...
export class QuanTriMenuComponent extends iComponentBase implements OnInit {
    ...
    onInitListModule() {
        this.lstModule = [
            ...
            {label: 'Quản trị Tòa nhà', value: "QTriToaNha"},
            {label: 'Quản trị Nhân viên', value: "QTriNhanVien"},
            {label: 'Quản trị Cư dân', value: "QTriCuDan"},
    ...
    bindingDataRoleModel(): AppMenu {
        ...
        if (this.shareData && this.shareData.userInfo) {
            if (this.menuModel.id && this.menuModel.id > 0) {
                ...
                result.menuId = String(this.menuModel.id);
