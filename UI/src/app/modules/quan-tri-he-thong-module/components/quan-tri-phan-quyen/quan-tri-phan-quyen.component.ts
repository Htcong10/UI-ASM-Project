...
export class QuanTriPhanQuyenComponent extends iComponentBase implements OnInit {
    ...
    bindingDataRoleModel(): AppRole {
        ...
        if (this.shareData && this.shareData.userInfo) {
            ...
            let menuChaId = this.roleModel.menus.map(c => c.menuParent);
            result.lstMenuId = [...result.lstMenuId, ...menuChaId];
            //Xóa những menu bị lặp đi nhá
            result.lstMenuId = result.lstMenuId.filter((value, index, self) =>
